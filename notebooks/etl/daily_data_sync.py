...
def validate_data(df: pl.DataFrame) -> pl.DataFrame:
    ...
    final_count = len(df)
    print(f"Validation: {initial_count} -> {final_count} rows ({initial_count - final_count} removed)")
...
def _(mo):
    ...
    source_input = mo.ui.text(
        value="data/sample/test.parquet",
        label="Source Path"
    )
    dest_input = mo.ui.text(
        value="data/output/test_output.parquet", 
