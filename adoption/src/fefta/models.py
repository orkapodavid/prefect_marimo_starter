...
class FeftaSource(BaseModel):
    ...
    saved_path: Optional[str] = Field(
        None, description="Local path where Excel was saved"
    )
...
class FeftaRecord(BaseModel):
    ...
    securities_code: str = Field(
        ..., description="証券コード (Securities code)", min_length=1
    )
    isin_code: str = Field(..., description="ISINコード (ISIN code)", min_length=1)
    company_name_ja: str = Field(
        ..., description="会社名（和名）- Japanese company name"
    )
