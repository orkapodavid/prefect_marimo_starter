...
Usage:
    from src.market_intelligence.fefta import FeftaCrawler
...
class FeftaCrawler:
    ...
    def fetch_latest_source(self) -> FeftaSource:
        ...
        if not fefta_links:
            raise FeftaLinkNotFoundError(
                f"No FEFTA Excel links found on page: {self.base_url}"
            )
    ...
    def parse_records(self, saved_path: str) -> Tuple[List[FeftaRecord], pd.DataFrame]:
        ...
        logger.info(
            f"Parsed {len(records)} records from Excel (skipped {skipped_rows} rows)"
        )
        return records, df
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            if "FEFTA" in link_text and href.endswith(".xlsx"):
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not parse date from link: {link_text[:80]}..."
                    )
                    continue
    ...
    def _parse_as_of_date(self, link_text: str) -> Tuple[str, date]:
        ...
        if not match:
            raise FeftaDateParseError(
                f"Could not find 'As of' date pattern in: {link_text}"
            )
        ...
        if month_key not in MONTH_MAP:
            raise FeftaDateParseError(
                f"Unknown month name '{month_name}' in: {link_text}"
            )
    ...
    def _parse_row(self, row: pd.Series, row_idx: int) -> FeftaRecord:
        ...
        category = self._normalize_circled_numeral(
            row.get("category", ""), row_idx, "category"
        )
    ...
    def _normalize_circled_numeral(
        ...
        if pd.isna(value) or value is None:
            raise FeftaExcelParseError(
                f"Empty value in column '{column_name}' at row {row_idx}"
            )
