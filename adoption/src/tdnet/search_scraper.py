...
class TdnetSearchScraper:
    ...
    SEARCH_TERMS = {
        "tier1": [
            {"query": "第三者割当 発行に関するお知らせ", "precision": "95%+", "description": "Initial issuance announcements"},
            {"query": "第三者割当 募集に関するお知らせ", "precision": "95%+", "description": "Initial offering announcements"},
        ],
        "tier2": [
            {"query": "第三者割当 新株式 -払込完了", "precision": "90%+", "description": "Common stock issuances (excluding completions)"},
            {"query": "第三者割当 新株予約権 -払込完了", "precision": "90%+", "description": "Warrant issuances (excluding completions)"},
        ],
        "tier3": [
            {"query": "第三者割当 割当先決定", "precision": "85%+", "description": "Allottee decision announcements"},
        ]
    }
    ...
    def scrape(self, start_date: Optional[date] = None, end_date: Optional[date] = None) -> TdnetSearchResult:
        """
                ...
                while page <= max_pages:
                    ...
                    if start_date and end_date:
                        ...
                        if page_dates and max(page_dates) < start_date:
                            logger.info(f"Reached data before start date on page {page}. Stopping query.")
                            break
                        ...
                        if key not in seen_keys:
                            ...
                            try:
                                ...
                                if self.download_pdfs and entry.pdf_link and HAS_PYPDF:
                                    pdf_text = self._download_and_extract_pdf(entry.pdf_link, entry.doc_id)
                                    if pdf_text:
    ...
    def _extract_deal_details(self, text: str) -> Dict[str, Any]:
        ...
        if investor: details['investor'] = investor.group(1).strip()
        ...
        if price: details['share_price'] = price.group(1).replace(',', '')
            
        count = re.search(r'発行新株式数[^0-9]*([0-9,]+)\s*株', text)
        if count: details['share_count'] = count.group(1).replace(',', '')
            
        d_match = re.search(r'(?:払込期日|割当日|発行日)[^0-9]*([0-9]{4})年([0-9]{1,2})月([0-9]{1,2})日', text)
        if d_match:
            details['deal_date'] = f"{d_match.group(1)}/{d_match.group(2)}/{d_match.group(3)}"
            
        if '新株予約権' in text: details['deal_structure'] = 'Warrant/Stock Option'
        elif '転換社債' in text: details['deal_structure'] = 'Convertible Bond'
        elif '新株式' in text: details['deal_structure'] = 'Common Stock'
