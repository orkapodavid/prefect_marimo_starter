...
class TdnetAnnouncement(BaseModel):
    ...
    stock_code: str = Field(
        ..., description="4-5 digit stock code", min_length=4, max_length=5
    )
    ...
    xbrl_url: Optional[str] = Field(
        None, description="Direct URL to XBRL zip file, JP only"
    )
...
class TdnetScrapeResult(BaseModel):
    ...
    language: TdnetLanguage = Field(
        TdnetLanguage.ENGLISH, description="Language of the scrape"
    )
