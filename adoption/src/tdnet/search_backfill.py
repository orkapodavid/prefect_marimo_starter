...
class TDnetPDFBackfill:
    ...
    def load_data(self):
        ...
        self.stats['missing_pdf'] = sum(1 for row in self.data if not row.get('pdf_link', '').strip())
        print(f"Loaded {self.stats['total']} entries, {self.stats['missing_pdf']} missing PDF links")
                ...
                if response.status_code == 200:
                    ...
                    for link in links:
                        ...
                        if '.pdf' in href:
                            ...
                            pdf_url = f"https://www.release.tdnet.info/inbs/{href}" if not href.startswith('http') else href
    ...
    def run(self, strategies: List[str] = None):
        ...
        self.stats['still_missing'] = sum(1 for row in self.data if not row.get('pdf_link', '').strip())
